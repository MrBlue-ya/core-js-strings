...
function removeFirstOccurrences(str, value) {
  const remove = str.indexOf(value);
  if (remove === -1) return str;
  return str.slice(0, remove) + str.slice(remove + value.length);
}
...
function removeLastOccurrences(str, value) {
  const remove = str.lastIndexOf(value);
  if (remove === -1) return str;
  return str.slice(0, remove) + str.slice(remove + value.length);
}
...
function isPalindrome(str) {
  const revStr = str
    ...
    .toLowerCase()
    .replace(/[^a-z]/gi, '');
  const strNew = str.replace(/[^a-z]/gi, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrSentence.length; ) {
    if (longestWord.length < arrSentence[i].length) {
...
function extractNameFromTemplate(value) {
  return value.split(', ')[1].replace(/[^a-z\s]/gi, '');
}
